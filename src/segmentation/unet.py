"""
U-Net model for vocal tract segmentation in rtMRI frames.

This implementation is adapted from BartsMRIPhysics/Speech_MRI_2D_UNet
to work with the USC-TIMIT dataset format.

Original Author: Matthieu Ruthven (matthieuruthven@nhs.net)
Adapted for Project Sullivan
"""
...
class UNet_n_classes(nn.Module):
    """
    U-Net architecture for N-class segmentation of rtMRI frames.

    Architecture:
    - Encoder: 5 layers with max pooling (64→128→256→512→1024 channels)
    - Decoder: 4 layers with transposed convolutions and skip connections
    - Features: Batch normalization, dropout (0.5), ReLU activation

    Args:
        n_classes: Number of segmentation classes (including background)
    """
    ...
    def forward(self, x):
        ...
        Args:
            x: Input tensor (B, 1, H, W) - single-channel grayscale images

        Returns:
            Output tensor (B, n_classes, H, W) - segmentation logits
        """
        # Layer 1 (encoding)
...
def load_pretrained_unet(weights_path: str, n_classes: int = 7, device: str = 'cuda') -> UNet_n_classes:
    """
    Load pre-trained U-Net model.

    Args:
        weights_path: Path to .pth file containing model weights
        n_classes: Number of segmentation classes (default: 7 for Barts dataset)
        device: Device to load model on ('cuda' or 'cpu')

    Returns:
        Loaded U-Net model in eval mode
    """
    model = UNet_n_classes(n_classes)
